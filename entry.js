...
(() => {
    ...
    const BASE_URL = CURRENT_SCRIPT?.src
        ? new URL('./', CURRENT_SCRIPT.src)
        : new URL('/scripts/extensions/third-party/Pocket-Phone/', window.location.origin);
            ...
            if ('caches' in window) {
                for (const cacheName of await window.caches.keys()) {
                    ...
                    for (const request of await cache.keys()) {
                        if (isPocketPhoneUrl(request.url) && await cache.delete(request)) report.cacheEntries += 1;
                    }
                }
            }
        } catch (error) {
    ...
    async function cleanAfterUpdateAndReload() {
        ...
        console.info(`[Pocket Phone ${VERSION}] Update detected (${previous || 'first install'} → ${VERSION}); reloading once after cleanup.`, report);
    ...
    function installObservers() {
        const attachVersion = () => patchVersion(document);
        const attachPanel = () => {
            ...
            panel.innerHTML = `
                <div class="inline-drawer">
                    ...
                    <div class="inline-drawer-content">
                        <div style="font-size:12px;opacity:.75;margin:6px 0;">Updates clean old code automatically. Manual recovery preserves all phone data.</div>
            ...
            button.addEventListener('click', async () => {
                if (!window.confirm('Clear old Pocket Phone code caches and reload?\n\nContacts, messages, feed, media, wallet data, and settings are preserved.')) return;
                button.disabled = true;
                status.textContent = 'Clearing old code cache…';
                try {
                    const report = await clearOldCodeCaches();
                    const stamp = `${VERSION}-${Date.now()}`;
                    try { window.sessionStorage.setItem(BUST_KEY, stamp); } catch {}
                    status.textContent = `Cleared ${report.scripts} scripts and ${report.cacheEntries} cached files. Reloading…`;
                    setTimeout(() => reloadWithMarker('pp_recovery', stamp), 300);
                } catch (error) {
                    button.disabled = false;
                    status.textContent = `Recovery failed: ${error?.message || error}`;
    ...
    window.PP_CLEAR_OLD_FILES = async () => {
        const report = await clearOldCodeCaches();
        reloadWithMarker('pp_recovery', `${VERSION}-${Date.now()}`);
        return report;
    };
    installObservers();
